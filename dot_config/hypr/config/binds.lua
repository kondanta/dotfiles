...
hl.bind(mainMod .. " + SHIFT + Up", function()
    ...
    hl.dispatch(hl.dsp.focus({ monitor = "u" }))
end)
hl.bind(mainMod .. " + SHIFT + Down", function()
    hl.dispatch(hl.dsp.window.move({ direction = "d" }))
    hl.dispatch(hl.dsp.focus({ monitor = "d" }))
end)
-- window-to-monitor moves: use SUPER+SHIFT+Up/Down (already bound above)
...
hl.bind(mainMod .. " + W",          hl.dsp.exec_cmd(launchPrefix .. BROWSER .. " -ProfileManager"))
...
-- Focus relative workspace N on current monitor (SUPER+1..9)
for i = 1, NUM_WPM do
    local key = i % 10
    hl.bind(mainMod .. " + " .. key, hl.dsp.focus({ workspace = "m~" .. i }))
end

-- Move window to relative workspace N and follow it (SUPER+SHIFT+1..9)
for i = 1, NUM_WPM do
    local key = i % 10
    hl.bind(mainMod .. " + SHIFT + " .. key, function()
        hl.dispatch(hl.dsp.window.move({ workspace = "m~" .. i }))
        hl.dispatch(hl.dsp.focus({ workspace = "m~" .. i }))
    end)
end
